...
pub fn step(_instruction: u32, registers: [i32; 31]) -> [i32; 31] {
    ...
    //let starting_pc_release_mode = 0x420001e8;
    //x30 = starting_pc_release_mode;
    //x31 = instruction[0];
    ...
    // Oh hmm DRAM vs IRMA https://www.espressif.com/sites/default/files/documentation/esp32-c3_technical_reference_manual_en.pdf#sysmem
    // Store access fault still, but sdkconfig has memprot off...... Is sdkconfig.defaults the
    // wrong place?
    //    let middle_ish_of_iram_i_hope = 0x41000000;
    //    x28 = middle_ish_of_iram_i_hope;
    ...
    unsafe {
        asm!(
                    // Of course store access fault
                    //"sw x31, 4(x30)", // Try to overwrite next instruction.
                    //"add x0,x0, x0", //NOP to overwrite.

                    //"addi x29, x0, 1", //0x10000
                  //  "slli x29, x29, 16",
                   // "lw x31, 2047(x0)",
                
                
               // "sw x30, 0(x28)", // Store instructions in middle(-ish) of IRAM 
               // "sw x29, 4(x28)",
               // "add x5, x1, x0",
               // "jalr x1, 0(x30)",
               // "add x1, x5, x0",
